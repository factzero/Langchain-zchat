...
from zchat.server.db.models.knowledge_base_model import KnowledgeBaseSchema
from zchat.server.db.repository.knowledge_base_repository import (
    add_kb_to_db,
    delete_kb_from_db,
    get_kb_detail,
    kb_exists,
...
from zchat.server.db.repository.knowledge_file_repository import (
    add_file_to_db,
    count_files_from_db,
    delete_file_from_db,
...
from zchat.server.knowledge_base.model.kb_document_model import DocumentWithVSId
...
from zchat.server.utils import (
...
class KBServiceFactory:
    ...
    def get_service(
        ...
        if SupportedVSType.FAISS == vector_store_type:
            from zchat.server.knowledge_base.kb_service.faiss_kb_service import (
                FaissKBService,
            )

            return FaissKBService(**params)
        elif SupportedVSType.PG == vector_store_type:
            from zchat.server.knowledge_base.kb_service.pg_kb_service import (
                PGKBService,
            )

            return PGKBService(**params)
        elif SupportedVSType.RELYT == vector_store_type:
            from zchat.server.knowledge_base.kb_service.relyt_kb_service import (
                RelytKBService,
            )

            return RelytKBService(**params)
        elif SupportedVSType.MILVUS == vector_store_type:
            from zchat.server.knowledge_base.kb_service.milvus_kb_service import (
                MilvusKBService,
            )

            return MilvusKBService(**params)
        elif SupportedVSType.ZILLIZ == vector_store_type:
            from zchat.server.knowledge_base.kb_service.zilliz_kb_service import (
                ZillizKBService,
            )
            ...
            from zchat.server.knowledge_base.kb_service.es_kb_service import (
                ESKBService,
            )

            return ESKBService(**params)
        elif SupportedVSType.CHROMADB == vector_store_type:
            from zchat.server.knowledge_base.kb_service.chromadb_kb_service import (
                ChromaKBService,
            )
