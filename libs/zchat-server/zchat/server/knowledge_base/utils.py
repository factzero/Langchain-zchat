...
from zchat.server.file_rag.text_splitter import (
    zh_title_enhance as func_zh_title_enhance,
)
from zchat.server.utils import run_in_process_pool, run_in_thread_pool
    ...
    try:
        if (
            ...
            try:  # 优先使用用户自定义的text_splitter
                text_splitter_module = importlib.import_module("zchat.server.file_rag.text_splitter")
...
def format_reference(kb_name: str, docs: List[Dict], api_base_url: str="") -> List[Dict]:
    ...
    将知识库检索结果格式化为参考文档的格式
    '''
    from zchat.server.utils import api_address
